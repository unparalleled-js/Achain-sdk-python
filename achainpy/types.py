...
from .exceptions import ErrBufferInvalidType, ErrInvalidSchema, ErrUnknownObj, ErrAbiProcessingError
import json
import binascii
import struct
import six
...
class BaseObject(object) :
    def __init__(self, d) :
        ...
        try:
            ...
            raise ErrInvalidSchema('Unable to process schema for {}'.format(type(self)))
...
class Asset :
    ...
    def _string_to_symbol(self):
        ...
        return ActBuffer(UInt64(rslt)).encode()

    def encode(self):
        ''' '''
        power = '1'.ljust(self.precision + len('1'), '0')
        amount = ActBuffer(UInt64(self.amount * UInt64(power))).encode()
...
class Abi(BaseObject):
    ...
    def get_action(self, name):
        ...
        for act in self.actions:
            if act.name == name:
                return act
        raise ErrUnknownObj('{} is not a valid action for this contract'.format(name))
    ...
    def get_struct(self, name):
        ...
        for struct in self.structs:
            if struct.name == name:
                return struct
        raise ErrUnknownObj('{} is not a valid struct for this contract'.format(name))
    ...
    def get_action_parameters(self, name):
        ...
        for field in struct.fields:
            ...
            if(f in self._abi_map):
                ...
                raise ErrUnknownObj("{} is not a known abi type".format(field.type))
        return parameters
    ...
    def json_to_bin(self, name, data):
        ...
        bin_buffer = ''
        for field in data:
            # create ActBuffer with value as a type of field
            if isinstance(params[field], list):
                ...
                field_buffer = ActBuffer(arr)
            else:
                field_type = type(params[field])
                field_buffer = ActBuffer(field_type(data[field]))
...
class PackedTransaction:
    ...
    def decode_actions(self, buf):
        ...
        while cnt < length and length:
            ...
            # get abi information
            contract_abi = self._cli.get_abi(acct_name)
            ...
            if abi_act["type"] != action_name:
                raise ErrAbiProcessingError("Error processing the {} action".format(action_name)) 
    ...
    def decode_context_actions(self, buf):
        ...
        if length > 0:
            raise NotImplementedError("Currently achainpy does not support context_free_actions")
        # get length of action array
    ...
    def decode_trx_extensions(self, buf):
        ...
        (length, ctx_buf) = self._decode_buffer(VarUInt(), buf)
        if length > 0:
            raise NotImplementedError("Currently achainpy does not support transaction extensions")
        # get length of action array
...
class ActBuffer :
    ...
    def decode(self, objType, buf=None):
        ...
        if isinstance(objType, UInt32):
            ...
            raise ErrBufferInvalidType("Cannot decode type: {}".format(type(objType)))
    ...
    def encode(self, val=None) :
        ...
        if (isinstance(val, Name) or
           ...
           isinstance(val, ScopeName) ) :
            ...
            raise ErrBufferInvalidType('Cannot encode type: {}'.format(type(val)))
